...
Vec3f TextureModelShader::vertexShader(int face, int vertIndex)
{
    ...
    Vec3f V = (M * Matrix::v2m(vertCoords[vertIndex])).toVec();
    Vec3f v = (vM * Matrix::v2m(vertCoords[vertIndex])).toVec();

    viewDir[vertIndex] = (eye - Vec3f(int(V.x), int(V.y), int(V.z))).normalize();
...
{      
    ...
    auto N = Matrix::v2m(model->normal(interpolatedUv));
    N[3][0] = 0.f;
    Vec3f n = (MIT * N).toVec().normalize();

    // n = model->normal(interpolatedUv);
